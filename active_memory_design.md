...
- **位置条件困惑度（long-context bucket test）**：100 篇长文档（每篇 ≥ 2048 tokens，截断至 2048，跳过前 40 万篇），用 `eval_long_context.py` 按 token 位置分桶对比。结果如下：

  | 位置区间 | n tokens | loss_b | loss_a | Δ loss | PPL_b | PPL_a | rel Δ% |
  |---|---|---|---|---|---|---|---|
  | [0, 64) | 6,400 | 4.5831 | 4.4271 | -0.1560 | 97.82 | 83.69 | -14.45% |
  | [64, 256) | 19,200 | 4.3326 | 4.1769 | -0.1557 | 76.14 | 65.16 | -14.42% |
  | [256, 512) | 25,600 | 4.2998 | 4.1314 | -0.1685 | 73.69 | 62.26 | -15.50% |
  | [512, 1024) | 51,200 | 4.3013 | 4.1156 | -0.1856 | 73.79 | 61.29 | -16.94% |
  | [1024, 2048) | 102,300 | 4.3777 | 4.1752 | -0.2025 | 79.66 | 65.05 | -18.33% |
  | **overall** | **204,700** | **4.3511** | **4.1628** | **-0.1882** | **77.56** | **64.25** | **-17.16%** |

  Δloss 随位置单调递增（更负）：`0→-0.156  64→-0.156  256→-0.169  512→-0.186  1024→-0.203`。开头两桶（位置 0–256）几乎持平，符合预期（上下文尚短，AM 无历史可利用）；从位置 256 起持续拉开，位置 1024–2048 的 AM 优势（-18.33%）比开头（-14.45%）大 ~30%。**支持"长程位置越远 AM 优势越大"的核心假设。** 这也弥补了远距回忆测试中 Δlogp 不单调的不足，为长程机制提供了更直接的证据。（硬件：A100）
